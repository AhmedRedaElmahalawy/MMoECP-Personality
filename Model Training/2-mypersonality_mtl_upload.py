# -*- coding: utf-8 -*-
"""mypersonality_mtl_upload.ipynb

Automatically generated by Colab.

Original file is located at
    https://colab.research.google.com/drive/12tNf1E8vxhMgJlY1R8yDIikLYyFcVzwu
...
data = pd.read_csv('mypersonality_preprocessed.csv')
data.dropna(inplace=True)
print("Data shape:", data.shape)

# Load the saved BERT embeddings
embeddings = joblib.load('bert_embeddings_STATUS_clean.pkl')
...
joblib.dump(y_pred_labels, 'y_pred_labels_transformer_MTL_BERT_STATUS_clean.pkl')
...
# -------- Save outputs --------
joblib.dump(y_pred_labels, 'y_pred_labels_MTL_CNN_STATUS_clean.pkl')
joblib.dump(thr, 'val_tuned_thresholds.pkl')
...
class MLPExpert:
    def __init__(self, hidden_layer_sizes=(200,3), activation='relu', alpha=1e-4, max_iter=200):
...
tf.random.set_seed(42)
# Load and preprocess the data
data = pd.read_csv('/content/mypersonality_preprocessed.csv')
data.dropna(inplace=True)
print("Data shape:", data.shape)
...
labels = data[['cEXT1', 'cNEU1', 'cAGR1', 'cCON1', 'cOPN1']]
# Load the saved BERT embeddings
embeddings = joblib.load('/content/bert_embeddings_STATUS_clean.pkl')
X = np.array(embeddings)
...
class LogisticRegressionExpert:
    def __init__(self, C=0.000001, max_iter=300, solver='liblinear'):
...
# SVC Experts (replacing MLP)
class SVCExpert:
    def __init__(self, C=0.2, kernel='rbf', gamma='scale'):
        # SVC must have probability=True to use predict_proba
        self.model = SVC(
            C=C,
            kernel=kernel,
            gamma=gamma,
...
class DecisionTreeExpert:
    def __init__(self, criterion='log_loss', splitter='random', max_depth=5, min_samples_split=2, min_samples_leaf=2,
